...
Result bfs_solver(long long input) {
    ...
    std::deque<Node> open;
    open.push_back(Node(input, 0, initialHeuristic, -1, 0)); // when inserted in LIFO not relevant
    ...
    while (!open.empty()) {
        ...
        for (int i = 0; i < possibleMoves.size(); ++i) {
            ...
            Node newNode = n.makeNode(n, possibleMoves[i], nextState, 0); // when inserted in LIFO not relevant
